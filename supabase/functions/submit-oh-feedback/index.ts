...
serve(async (req) => {
  ...
  try {
    ...
    const rowsHtml = rows.map(([k, v]) => `<tr><td style="padding:6px 12px 6px 0;color:#8b7a68;font-size:13px;width:160px;">${k}</td><td style="padding:6px 0;color:#3A2A1E;font-size:14px;"><strong>${v}</strong></td></tr>`).join('')
    const blocksHtml = textBlocks.map(([k, v]) => `<div style="margin:14px 0;"><div style="color:#8b7a68;font-size:13px;margin-bottom:4px;">${k}</div><div style="color:#3A2A1E;font-size:14px;line-height:1.6;background:#F6F4EE;padding:10px 12px;border-radius:6px;">${v.replace(/\n/g, '<br/>')}</div></div>`).join('')
...
<div style="font-family: 'Nunito Sans', Arial, sans-serif; max-width: 600px; margin: 0 auto; color: #3A2A1E;">
  ...
  <p style="margin:0 0 16px;color:#5A4136;font-size:14px;">${addr}${dateLabel ? ` · ${dateLabel}` : ''}</p>
  <p style="margin:0 0 18px;color:#5A4136;font-size:14px;">From: <strong>${hostName}</strong>${fb.hosting_agent_email ? ` (${fb.hosting_agent_email})` : ''}</p>
