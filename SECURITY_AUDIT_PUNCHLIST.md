...
### [~] C7. `send-one-off-email` is a public phishing kit (DKIM-signed danamassey.com) — PARTIALS SHIPPED 2026-06-04, FULL FIX AUTH-BLOCKED
> Shipped (commit pending): defense-in-depth partials that cut the phishing-kit blast radius without breaking the SPA. (1) **Per-IP rate limit** via the M18 `check_rate_limit` RPC, 200/hr — covers a 150-recipient seller blast (sendListingEmailBlast loops sequentially) while capping a curl-loop attack. (2) **CRLF strip + 200-char cap** on the Subject — RFC 5322 header injection guard, also feeds the contact-note log. (3) **Email-shape regex** on `to_email` so payloads disguised as addresses get a 400 instead of reaching Resend. (4) **100KB HTML cap** so a runaway body can't inflate the Resend bill or wedge the 60s wall. (5) **Optional caller-supplied `idempotency_key`** wired into all 4 SPA callers: SendEmailModal, OneOffEmailComposer, ClosingRecapModal (per-modal-mount UUID that rotates after each successful send) and supabase.js `sendListingEmailBlast` (`blast_${contact.id}_${subjectPrefix}` per recipient). Double-clicks and SPA reloads mid-blast no longer double-send. Edge fn needs redeploy. **Still open:** server-side identity check (`db.auth.getUser(token)`) + enforce `to_email` belongs to a contact the caller owns — both require the multi-day [[project_auth_queued]] work.
> Original audit notes (2026-06-04): The audit-recommended fix is "verify user JWT server-side" but no Auth is wired yet. Service-role-only (C2/C8 pattern) would break every one-off email surface. Real fix lands as part of [[project_auth_queued]].
...
### [x] C10. Multiple functions bypass `ai-bill.ts` — wallet attack via unauthenticated `ai-assistant-chat` ✅ 2026-06-04 (budget cap part) + 2026-06-04 broader sweep + 2026-06-04 generate-content sweep
> Final pass (commit pending): generate-content's 4 direct fetch sites (listing_plan checklist + strategy, listing_content per-channel JSON, refine, generic) all routed through `callAnthropic`. `Promise.allSettled` replaces the previous `.ok` branching so a `budget_exceeded` rejection short-circuits cleanly. All 4 call sites now carry `feature: 'generate-content:<kind>'` and `attributedTo: { kind: 'listing', id }` for cost rollups. CI grep (`scripts/check-direct-llm-calls.sh`) now runs clean against the entire `supabase/functions/` tree. With this batch C10 is fully closed.
> Broader sweep (commit c2a78c4): 7 more files routed through callAnthropic — cma-parse, oh-approval-gate, gmail-client-email-monitor, ai-campaign-insights, generate-embeddings, build-gamma-custom, build-presentation, gmail-showing-monitor (both `claudeFallbackParse` and `analyzeSentiment` sites). gmail-showing-monitor's sentiment call also got `<showing-feedback>` delimiter tags + treat-as-data instruction (matches H8 fix pattern).
> First pass (commit c9ecfcf): ai-assistant-chat, auto-generate-content, embed-on-insert.
...
### [~] M1. CORS `*` on every edge function — state-mutating endpoints should be locked to production origin ✅ 2026-06-04 (helper + 13 high-value endpoints locked; broader sweep available via helper)
> Shipped: new `_shared/cors.ts` exports `corsHeadersFor(origin)` which echoes back the request Origin if it's in the allowlist (`app.danamassey.com`, Vercel preview, localhost) and `null` otherwise. Plus legacy `CORS_OPEN` for webhook receivers that don't have a useful Origin. Now applied to 13 state-mutating SPA endpoints: `ai-assistant-chat`, `send-one-off-email`, `generate-image`, `virtual-staging`, `higgsfield-generate`, `generate-content`, `publish-content`, `elevenlabs-tts`, `fetch-social-stats`, `build-presentation`, `build-gamma-custom`, `cma-parse`, `google-drive` (covers all the high-cost vendor-spend and Drive-write paths). Webhook receivers (resend-webhook, lofty-webhook, embed-on-insert), OAuth callbacks (google-auth), public submission endpoints (merge-form-submission, merge-oh-signin, submit-oh-feedback, host-report-followup, biolink-submit) deliberately stay on `CORS_OPEN` because they have no useful browser Origin. Defense-in-depth only — CORS is browser-enforced, so direct curl/script attacks still get through (bearer/budget/signature checks own the real enforcement). Edge fns need redeploy.
...
### [~] M2. No `fetch()` timeouts (frontend + edge fns) ✅ 2026-06-04 (highest-blast-radius spots done + frontend external API sweep)
> Shipped: (1) `AbortSignal.timeout(25_000)` on the Anthropic call in `_shared/ai-bill.ts` (every LLM call across the codebase now benefits — a hung Anthropic call can't burn the 60s edge fn budget). (2) `AbortSignal.timeout(45_000)` on the ElevenLabs TTS fetch. (3) New `frontend/src/lib/net.js` exports `fetchWithTimeout(url, opts, ms=15000)` + `isTimeoutError(err)` for use across the SPA. **Frontend sweep (commit pending):** every direct `fetch()` to an external API in the SPA now uses `fetchWithTimeout`. Weather (NWS) 10s in `useWeatherForecast`, `useWeatherAlerts`, `WeatherPrepFlags`; Airtable 15s in `airtable.js` (fetchAll + create + update); Blotato + Gamma 10s connectivity tests in `Settings.jsx`; `generate-content` edge-fn calls 45s in `EmailBuilder.jsx` and 45/60s in `Sellers.jsx` (offer analysis + offer comparison); `send-campaign-step` 30s in `SmartCampaigns.jsx`. Remaining direct `fetch` calls in the SPA are to internal Supabase functions that already have their own request-level guards (and most go through `supabase.functions.invoke` which has its own client-side timeout).
