...
export const useOfferHistory  = (contactId) => useQuery(() => contactId ? DB.getOfferHistory(contactId) : Promise.resolve([]), [contactId])
...
export const useExpensesForListing = (listingId) => useQuery(() => listingId ? DB.getExpensesForListing(listingId) : Promise.resolve([]), [listingId])
export const useExpensesForContact = (contactId) => useQuery(() => contactId ? DB.getExpensesForContact(contactId) : Promise.resolve([]), [contactId])
...
export const useCommsForContact     = (cid) => useQuery(() => cid ? DB.getCommsForContact(cid) : Promise.resolve([]), [cid])
export const useLatestComms         = ()    => useQuery(DB.getLatestComms)
export const useResources           = ()    => useQuery(DB.getResources)
export const useFormSendsForContact = (cid) => useQuery(() => cid ? DB.getFormSendsForContact(cid) : Promise.resolve([]), [cid])
export const useActiveEnrollments  = ()    => useQuery(DB.getActiveEnrollments)
export const useFbAudiences        = ()    => useQuery(DB.getFbAudiences)
