...
export default function FsboLeadsDb() {
  const [leads, setLeads] = useState([])
  const [loading, setLoading] = useState(true)
  const [showAdd, setShowAdd] = useState(false)
  const [saving, setSaving] = useState(false)
  const [filter, setFilter] = useState('all')
  const [draft, setDraft] = useState({ address: '', city: '', listed_price: '', data_source: 'manual' })

  useEffect(() => { load() }, [])

  async function load() {
    setLoading(true)
    const { data } = await supabase.from('fsbo_leads')
      .select('*, property:properties(id, address, city, mls_id)')
  ...
  const handleAdd = async () => {
    ...
    try {
      ...
      if (prop) {
        await supabase.from('fsbo_leads').insert({
          property_id: prop.id,
      ...
      setShowAdd(false)
      setDraft({ address: '', city: '', listed_price: '', data_source: 'manual' })
      await load()
  ...
  const handleStatusChange = async (id, status) => {
    await supabase.from('fsbo_leads').update({ status, updated_at: new Date().toISOString() }).eq('id', id)
    await load()
  }

  if (loading) return <p style={{ color: 'var(--color-text-muted)', padding: 16 }}>Loading FSBO leads...</p>
    ...
    <div>
      <div style={{ display: 'flex', gap: 6, marginBottom: 12, flexWrap: 'wrap', alignItems: 'center' }}>
        ...
        <div style={{ marginLeft: 'auto' }}>
          <Button size="sm" onClick={() => setShowAdd(!showAdd)}>+ Add FSBO</Button>
        </div>
      </div>
      ...
      {showAdd && (
        <div style={{
          ...
          <div style={{ display: 'flex', gap: 6 }}>
            <Input label="Asking Price" type="number" value={draft.listed_price} onChange={e => setDraft(d => ({ ...d, listed_price: e.target.value }))} placeholder="425000" style={{ flex: 1 }} />
            <Select label="Source" value={draft.data_source} onChange={e => setDraft(d => ({ ...d, data_source: e.target.value }))} style={{ flex: 1 }}>
              <option value="manual">Manual</option>
              <option value="zillow">Zillow FSBO</option>
              <option value="realtor">Realtor.com</option>
              <option value="facebook">Facebook Marketplace</option>
              <option value="driving">Driving for Dollars</option>
            </Select>
          </div>
          <div style={{ display: 'flex', gap: 6 }}>
      ...
      {filtered.length === 0 ? (
        <p style={{ color: 'var(--color-text-muted)', fontSize: '0.82rem', padding: '16px 0' }}>
          {leads.length === 0 ? 'No FSBO leads in the database yet.' : 'No leads match this filter.'}
        </p>
      ) : (
              ...
              <div key={lead.id} style={{
                ...
                <div style={{ flex: 1 }}>
                  ...
                  <div style={{ fontSize: '0.68rem', color: 'var(--color-text-muted)', marginTop: 2 }}>
                    ...
                    {lead.data_source !== 'manual' ? ` · via ${lead.data_source}` : ''}
                ...
                <select
                  ...
                  style={{ fontSize: '0.72rem', padding: '4px 8px', borderRadius: 4, border: '1px solid var(--color-border)', background: '#fff' }}
                >
